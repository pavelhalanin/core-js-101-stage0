...
function doRectanglesOverlap(rect1, rect2) {
  const Ax = rect1.left;
  const Ay = rect1.top;
  const Bx = rect1.left + rect1.width;
  const By = rect1.top + rect1.height;

  const Cx = rect2.left;
  const Cy = rect2.top;
  const Dx = rect2.left + rect2.width;
  const Dy = rect2.top + rect2.height;

  const IS_OVERLAP = Bx > Cx && Dx > Ax && By > Cy && Dy > Ay;
  return IS_OVERLAP;
}
...
function isInsideCircle(circle, point) {
  ...
  const IS_IN_CIRCLE = (X - X0) ** 2 + (Y - Y0) ** 2 < R ** 2;
  return IS_IN_CIRCLE;
}
...
function findFirstSingleChar(str) {
  const ARRAY = [];
  const ARRAY_CHARS = `${str}`.split('');

  for (let i = 0; i < ARRAY_CHARS.length; i += 1) {
    let isFound = false;
    const CHAR = ARRAY_CHARS[i];
    for (let j = 0; j < ARRAY.length; j += 1) {
      if (ARRAY[j].char === CHAR) {
        ARRAY[j].count += 1;
        isFound = true;
        break;
      }
...
function getDigitalRoot(num) {
  ...
  do {
    ...
    n = arr.reduce((counter, element) => counter + element, 0);
  } while (arr.length > 1);
  return n;
}
...
function getCommonDirectoryPath(pathes) {
  let result = '';
  const ARRAY = pathes.map((e) => e.split('/'));
  const MAX = Math.max(...ARRAY.map((e) => e.length));
