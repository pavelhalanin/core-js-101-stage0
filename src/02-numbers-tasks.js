...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const X = x1 * x2; // X = x₁ * x₂
  const Y = y1 * y2; // Y = y₁ * y₂
  const lXl = Math.sqrt(x1 ** 2 + y1 ** 2); // |X| = √(x₁² + x₂²)
  const lYl = Math.sqrt(x2 ** 2 + y2 ** 2); // |Y| = √(y₁² + y₂²)
...
function getParallelepipedDiagonal(a, b, c) {
  return Math.sqrt(a ** 2 + b ** 2 + c ** 2); // V = √(a² + b² + c²)
}
...
function toNumber(value, def) {
  const NUMBER = +value;
  const IS_NUMBER = typeof NUMBER === 'number' && !Number.isNaN(NUMBER);
  return IS_NUMBER ? NUMBER : def;

  /*
  const IS_NUMBER = /^-?\d+(\.\d+)?$/.test(`${value}`);
  return IS_NUMBER ? value : def;
  */
}
